...
describe("Сценарии создания JWT токена", function() {
    const builder = new AuthTestDataBuilder();
    ...
    context("Сценарий создания JWT токена", function() {
        const password = "Pas77Wo88!";
        const email = "gachi@tarkov.gay";
        const userId = 323;
        ...
        before(async function() {
            manualResetTestInstances();
            const passwordHash = await CryptoHelper.getHash(password);

            builder
                // говорим чтоб userdb по запросу пароля через почту, возвращал ранее посчитанный хэш и имел id-шник
                .addStep((d) => {
                    d.usersStub.addMock((user) => when(user.passwordHash).thenReturn(passwordHash))
        ...
        it("Удачный сценарий", async function() {
            ...
            const mockedServiсe = builder.Data.userService.proxy;
    ...
    context("Использование refresh token-а для получения нового", function() {
        ...
        const refreshToken = "StubRefreshTokenABRAKADABRA";
        ...
        // TODO: удалил требование на входе протухшего access Токена
        // из него до этого раньше брался userId для всех дальнейших действий
        // как доделаешь аснки с логином, вернись сюда
